...
from .. core   .fit_functions             import sigmoid
...
def hist2D(df        : pd.DataFrame,
    ...
    values, ebins, _  = stats.binned_statistic_dd((df.X, df.Y),
                                                  df.Ec_2,
                                                  bins=[np.linspace(*xrange, bins),
                                                  np.linspace(*yrange, bins)],
                                                  statistic = statistic)
    ...
    h, xedges, yedges, im = axs.hist2d(x_grid,
                                       y_grid,
                                       bins=ebins,
                                       weights=weight,
                                       cmin=25,
...
def plot_Ec(Ec   : pd.core.series.Series,
    ...
    mean_Ec    = Ec.mean()
    stdEc      = Ec.std()
    umeanEc    = Ec.std()/np.sqrt(len(Ec_2))
    median_Ec  = Ec.median()

    fig, axs = plt.subplots(1, 1)
    axs.hist(Ec, 100, range = (25, 60), histtype = 'step', color = 'black', lw = 2,
...
def plot_time_evolution_with_errors_and_dates(df_time_evolution : pd.DataFrame,
    ...
    try:
        ...
        data_to_plot = {
            ...
            'ec_self':  {'value_col': 'ec_self', 'error_col': 'ec_selfu','units': 'keV'},
...
def make_control_plots(df               : pd.DataFrame,
    ...
    plot_Ec(df_corr['Ec'], df_corr['Ec_2'])
    ...
    plot_efficiencies(efficiencies)
