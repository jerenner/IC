"""
This script contains functions to compute empty maps, 3D maps from
kdsts and merge them to create an 'uniform' map.  It also contains
...
from .. icaros.lifetime_vdrift_functions import compute_drift_v
...
def create_empty_map(xy_range : tuple,
    ...
    ----------
    xy_range : tuple
      Range in x and y inside which the map is being computed.
    dt_range : tuple
      Range in drift time inside which the map is being computed.
    xy_nbins : int
    ...
    Nan_map : pd.DataFrame
      Dataframe filled with NaNs for each combination of z,x,y bins.
    """
...
def get_median(var : np.ndarray) -> pd.DataFrame:
    """
    Computes the median value of a given variable
    -We consider that for len(df) < 5, it makes no sense to compute the median or the std.
    ...
    return pd.DataFrame({'nevents'    : len(var),
                         'mu'         : mu,
                         'sigma'      : sigma,
...
def gaussian_fit(var        : np.ndarray,
                 nbins      : int,
                 min_events : int = 50) -> pd.DataFrame:
    """
    Given a dataframe, computes a gaussian fit of a given variable if len(df) is equal or
    greater than min_events. If not, computes the median using get_median.

    If the mean value from the gaussian fit value is not between [0.5*median, 1.5*median], the median is computed using get_median.
    ...
    map : pd.DataFrame
      ...
      of the fit.
    """
    if len(var)< min_events:
        return get_median(var)

    counts, bin_edges = np.histogram(var, nbins)
    ...
    if not in_range(f.values[1], 0.5*var.median(), 1.5*var.median()):
        return get_median(var)
...
def fit_map(df            : pd.DataFrame,
            ...
            dt_nbins      : int,
            S2e_range     : tuple,
            fit_function  : Callable) -> pd.DataFrame:
    """
        ...
        for i in range(1, mu.shape[1] -1):
            for j in range(1, mu.shape[2] -1):
                if np.isnan(mu[k,i,j]):
                    ...
                    if np.count_nonzero(np.isnan(vals)) > max_nan_neighbours:
                        continue
...
def compute_3D_map(df           : pd.DataFrame,
                   ...
                   fit_function : Callable) -> pd.DataFrame:
    """
    Computes a 3D map from the data in df given a range in xy and dt and the number of bins for each coordinate.
    The map values in each voxel are computed using fit_function.
    A data quality filter is applied requiring S2e to be in a given range S2e_range.
    ...
    map_3D_fit = fit_map(df, xy_range, dt_range, xy_nbins, dt_nbins, S2e_range, fit_function)
    ...
    t_hours = (df.time.values.max() - df.time.values.min())/3600 #from seconds to hours
...
def get_time_evol_single_slice(df           : pd.DataFrame,
    ...
    try:
        f2                 = quick_gauss_fit(df['Ec_2'].values, bins_Ec, sigma = error)
        _, mu2, sigma2     = f2.values
        _, u_mu2, u_sigma2 = f2.errors

        chi2_ec_self       = f2.chi2

    except:
        median2      = get_median(df['Ec_2'])
        mu2          = median2.mu
    ...
    R   = np.sqrt(8*np.log(2))*(sigma2/mu2) #FWHM (krypton paper)
    u_R = R * (u_mu2**2/mu2**2 + u_sigma2**2/sigma2**2)**0.5
    ...
    t_evol = {'run_number'     : run_number,
              ...
              'ec_self'        : mu2,
              'ec_selfu'       : u_mu2,
              'chi2_ec_prelim' : chi2_ec_prelim,
              'chi2_ec_self'   : chi2_ec_self,
              'e0'             : e0,
              'e0u'            : e0u,
              'lt'             : lifetime ,
              'ltu'            : ulifetime,
...
def get_time_evol(df          : pd.DataFrame,
    ...
    time_bins = np.arange(df.time.min(), df.time.max() + slice_seconds, slice_seconds)
