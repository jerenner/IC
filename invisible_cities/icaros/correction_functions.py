...
def normalization(krmap     : pd.DataFrame,
    ...
    raise ValueError(f'Unsuported NormMethod: {method}')
...
def apply_3Dmap(krmap       : pd.DataFrame,
                ...
                y           : np.ndarray,
                E           : np.ndarray,
                xy_params   : Union[dict,  NoneType] = None,
                unit        : Union[float, NoneType] = keV) -> np.ndarray:
    """
...
def apply_correctionmap_inplace_kdst(kdst        : pd.DataFrame,
    ...
    """
    corrected_energy = apply_3Dmap(map3D, norm_method, kdst.DT, kdst.X, kdst.Y, kdst.S2e, xy_params = xy_params, unit = unit)
...
def apply_correctionmap_inplace_hits(hdst        : pd.DataFrame,
    ...
    """
    corrected_energy = apply_3Dmap(map3D, norm_method, hdst.Z, hdst.X, hdst.Y, hdst.E, xy_params = xy_params, unit = unit)
